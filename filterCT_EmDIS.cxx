...
int main(int argc, char **argv)
{
    ...
    int minRows = MAX_ELECTRONS; // number of particles in an event to start filtering
    ...
    while (k < MaxEvents) {
      ...
      if(nRows >= minRows){
        ...
        for (j = 0; j < nRows; j++) {
          ...
          if(catPid.EqualTo("high energy pion +") || catPid.EqualTo("low energy pion +") || catPid.EqualTo("pi+")) myKine.nPip++;
          if(catPid.EqualTo("pi-")) myKine.nPim++;
          if(catPid.EqualTo("gamma")) myKine.nGam++;
        ...
        topology = (myKine.nElec>=MAX_ELECTRONS); // check event topology
